...
def test_grouped_coercion_filters_minus100(
    monkeypatch, qrels_flagged, run_with_flagged_pairs
):
    ...
    pipelines, names = BRIGHT.coerce_pipelines_grouped(
        ctx, pipeline_generators=[Identity()]
    )
...
def test_sequential_coercion_filters_minus100(
    monkeypatch, qrels_flagged, run_with_flagged_pairs
):
    ctx = _DummyContext(qrels_flagged)
    ...
    monkeypatch.setattr(
        Suite, "coerce_pipelines_sequential", _fake_sequential, raising=True
    )
